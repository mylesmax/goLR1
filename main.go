...
func main() {
	...
	cur.out = struct {
		V, m, h, j, d, f, X, K1, Xi, Kp, stim []float64
		...
		Kp: []float64{cur.gates.Kp.val}, stim: []float64{0}, INa: []float64{0}, Isi: []float64{0}, Ik: []float64{0}, IK1: []float64{0}, IKP: []float64{0}, Ib: []float64{0},
		Cai: []float64{c.Cai}}
	...
	for i, t := range tspan {
		...
		cur.out.stim = append(cur.out.stim, cur.stim)
		...
		out += "   "
		out += strconv.FormatFloat(cur.out.stim[i], 'f', 7, 64)
	...
	fmt.Println("Simulation complete. Outputted variables: t, V, m, h, j, d, f, X, Xi, K1, Kp, INa, Isi, Ik, IK1, IKP, Ib, and stimulus.")
}
